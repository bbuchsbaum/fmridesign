...
test_that("internal event() handles duration recycling", {
    ...
    onsets <- c(1,2,3,4)
    blockids <- c(1,1,2,2)
    ev_d0 <- fmridesign:::event(vals, "d0", onsets, blockids) # Default duration 0
    expect_equal(ev_d0$durations, rep(0, 4), ignore_attr = TRUE)
       
    ev_d1 <- fmridesign:::event(vals, "d1", onsets, blockids, durations = 2)
    expect_equal(ev_d1$durations, rep(2, 4), ignore_attr = TRUE)
    
    ev_dvec <- fmridesign:::event(vals, "dvec", onsets, blockids, durations = c(1,1,5,5))
    expect_equal(ev_dvec$durations, c(1,1,5,5), ignore_attr = TRUE)
    
    # Error for wrong length duration
    expect_error(fmridesign:::event(vals, "derr", onsets, blockids, durations = 1:3))
})
...
test_that("event_term persists condition metadata for categorical terms", {
  onsets <- c(0, 10, 20)
  cond <- factor(c("A", "B", "A"), levels = c("A", "B"))
  term <- event_term(list(cond = cond), onsets = onsets, blockids = rep(1, 3))

  expect_equal(term$condition_levels, c("cond.A", "cond.B"))
  expect_equal(term$condition_ids, c(1L, 2L, 1L))

  ec <- event_conditions(term)
  expect_s3_class(ec, "factor")
  expect_equal(levels(ec), c("cond.A", "cond.B"))
  expect_equal(as.integer(ec), term$condition_ids)

  ec_drop <- event_conditions(term, drop.empty = TRUE)
  expect_equal(levels(ec_drop), c("cond.A", "cond.B"))
  expect_equal(as.integer(ec_drop), term$condition_ids)
})

test_that("event_conditions drop empty levels when requested", {
  onsets <- c(0, 10, 20)
  cond <- factor(c("A", "A", "A"), levels = c("A", "B"))
  term <- event_term(list(cond = cond), onsets = onsets, blockids = rep(1, 3))
  ...
  ec_drop <- event_conditions(term, drop.empty = TRUE)
  expect_equal(levels(ec_drop), c("cond.A"))
  expect_true(all(as.integer(ec_drop) == 1L))
})

test_that("event_conditions handles crossed categorical factors", {
  onsets <- c(0, 10, 20)
  cond <- factor(c("A", "B", "A"), levels = c("A", "B"))
  grp  <- factor(c("X", "X", "Y"), levels = c("X", "Y"))
  term <- event_term(list(cond = cond, grp = grp), onsets = onsets, blockids = rep(1, 3))

  expect_equal(term$condition_levels,
               c("cond.A_grp.X", "cond.B_grp.X", "cond.A_grp.Y", "cond.B_grp.Y"))

  ec <- event_conditions(term)
  expect_equal(levels(ec), term$condition_levels)
  expect_equal(as.integer(ec), c(1L, 2L, 3L))
})

test_that("event_conditions defaults to single condition for continuous terms", {
  onsets <- c(0, 10, 20)
  term <- event_term(list(mod = rnorm(3)), onsets = onsets, blockids = rep(1, 3))

  expect_equal(term$condition_levels, "condition")
  expect_equal(term$condition_ids, rep(1L, 3))

  ec <- event_conditions(term)
  expect_equal(levels(ec), "condition")
  expect_true(all(as.integer(ec) == 1L))
})

test_that("events() exposes canonical per-event information", {
  onsets <- c(0, 10, 20)
  cond <- factor(c("A", "B", "A"), levels = c("A", "B"))
  term <- event_term(list(cond = cond), onsets = onsets, blockids = c(1, 1, 2))

  evt <- events(term)
  expect_named(evt, c("onset", "duration", "block", "condition"))
  expect_equal(evt$onset, onsets)
  expect_equal(evt$block, c(1L, 1L, 2L))
  expect_s3_class(evt$condition, "factor")
  expect_equal(evt$condition, event_conditions(term))
})

test_that("column_groups_by_condition splits indices by basis", {
  onsets <- c(0, 10, 20)
  cond <- factor(c("A", "B", "A"), levels = c("A", "B"))
  term <- event_term(list(cond = cond), onsets = onsets, blockids = rep(1, 3))

  groups <- column_groups_by_condition(term, fmrihrf::HRF_SPMG2, NULL)
  expect_named(groups, c("cond.A", "cond.B"))
  expect_equal(groups[[1]], 1:2)
  expect_equal(groups[[2]], 3:4)

  groups_single <- column_groups_by_condition(term, fmrihrf::HRF_SPMG1, NULL)
  expect_named(groups_single, c("cond.A", "cond.B"))
  expect_equal(groups_single[[1]], 1)
  expect_equal(groups_single[[2]], 2)
})
