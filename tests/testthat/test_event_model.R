...
test_that("contrast_weights.event_model works with new attributes", {
  ...
  cw_list <- contrast_weights(model)
...
test_that("event_model handles hrf_bspline basis correctly", {
  ...
  cw_list2 <- contrast_weights(model_contrast_bspline2)
...
test_that("Golden heading test: diverse terms produce expected colnames", {
  ...
  expect_identical(
    ...
    c(
      ...
      # multi-basis columns are condition-major (issue #23)
...
test_that("event_model list interface supports mixed bases (Poly + BSpline + Std)", {
  ...
  expected_term_tags <- c("COND", "MOD", "poly_x", "std_y")
  ...
  # Standardized(y): std_y_y (Now returns argname 'y')
  expect_true("std_y_y" %in% actual_colnames)
...
test_that("event_model correctly names columns for Ident() and combined basis functions", {
  ...
  # Expected: bs_Age_01, bs_Age_02, bs_Age_03
  expect_setequal(cols_bs_plain, c("bs_Age_01", "bs_Age_02", "bs_Age_03"))
  
  # Default name for BSpline(Age,3) -> term_tag = "bs_Age"
  model_bs_hrf <- event_model(onset ~ hrf(BSpline(Age, 3), basis="spmg2"), 
                              data = events, block = ~block, sampling_frame = sf)
  cols_bs_hrf <- colnames(design_matrix(model_bs_hrf))
  # Expected: bs_Age_01_b01 ... bs_Age_03_b02
  expected_bs_hrf_cols <- c(
    "bs_Age_01_b01", "bs_Age_01_b02", 
    "bs_Age_02_b01", "bs_Age_02_b02",
    "bs_Age_03_b01", "bs_Age_03_b02"
  )
