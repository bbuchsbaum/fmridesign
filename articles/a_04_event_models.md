...
An **event model** describes the expected BOLD signal changes related to
experimental events (stimuli, conditions, responses, etc.). It forms the
core of the task-related component of an fMRI General Linear Model
(GLM).

`fmridesign` primarily uses the
...
[`event_model()`](https://bbuchsbaum.github.io/fmridesign/reference/event_model.md).
There’s also a list-based interface (`event_model.list`) and a more
programmatic `create_event_model()` function for advanced use cases.
...
Consider a basic design with four stimulus types (*face*, *scene*,
*tool*, *object*), each presented 4 times for 2s, separated by a
...
Now, we create the `event_model`. The formula `onset ~ hrf(stim)`
specifies that:

- `onset` is the column in `simple_design` containing event start times.
- `hrf(stim)` defines a term where the `stim` factor levels determine
  the conditions. Each level will be convolved with the default HRF
  (`HRF_SPMG1`).

The `block = ~ run` argument links events to scanning runs (here, just
one run).
...
We can plot the generated regressors using
[`plot()`](https://rdrr.io/r/graphics/plot.default.html) (which uses
...
![Event model regressors for a simple single-factor design plotted over
...
The plot shows the predicted BOLD timecourse for each condition (`stim`
level) convolved with the default HRF.
...
The `event_model` call remains the same, but now `block = ~ run`
correctly maps events to their respective runs based on the `run` column
...
![Event model regressors across multiple runs plotted over
time.](a_04_event_models_files/figure-html/create_multi_block_model-1.png)

## Two-Factor Design

Now consider a design crossing stimulus type (`stim`) with task
...
#> 6  scene ignore  14   1    70
```

The formula `onset ~ hrf(stim, task)` automatically creates regressors
for the interaction of `stim` and `task`. The term tag will default to
`stim_task`.
...
![Interaction term regressors from a two-factor design plotted over
...
the modulating variable within the
[`hrf()`](https://bbuchsbaum.github.io/fmridesign/reference/hrf.md)
call.
...
#> 6  scene  12   1     6    96 595.0823  -127.08808
```

Here, `hrf(stim)` creates the main effect term (tag: `stim`), and
`hrf(RT_centered)` creates the parametric modulator term (tag:
`RT_centered`).
...
![Parametric modulator regressors (and main effects) plotted over
...
vs. scenes? The formula `hrf(stim, RT_centered)` creates separate RT
modulators for each level of `stim`.

Here, `hrf(stim)` is one term (tag: `stim`). The interaction
...
![Interaction of factor levels with parametric modulator plotted over
...
By default,
[`hrf()`](https://bbuchsbaum.github.io/fmridesign/reference/hrf.md) uses
the SPM canonical HRF (`HRF_SPMG1`). You can specify a different HRF
basis for any term using the `basis` argument within the
...
2.  **Using Pre-defined `HRF` Objects:** Pass an exported `HRF` object
    directly. Examples include `HRF_SPMG1`, `HRF_GAUSSIAN`, `HRF_SPMG2`,
    `HRF_SPMG3`, `HRF_BSPLINE`, `HRF_DAGUERRE`, `HRF_DAGUERRE_BASIS`.

3.  **Using Custom Functions `f(t)`:** Provide your own function
    definition that accepts time `t` as the first argument.

4.  **Using
    [`gen_hrf()`](https://bbuchsbaum.github.io/fmrihrf/reference/gen_hrf.html)
    Results:** Create a customized HRF using
    [`gen_hrf()`](https://bbuchsbaum.github.io/fmrihrf/reference/gen_hrf.html),
    `hrf_blocked()`, `hrf_lagged()`, etc., and pass the resulting `HRF`
    object.

**Discovering Available Options:** You can easily explore all available
...
For more details and examples of different HRF types, refer to the
documentation for
[`hrf()`](https://bbuchsbaum.github.io/fmridesign/reference/hrf.md) and
the HRF-related vignettes in the `fmrihrf` package.
...
in the formula. This creates a separate regressor for every single event
...
(drift/intercepts/nuisance) to form a full design matrix; downstream GLM
fitting happens in analysis packages (e.g., fmrireg).

## Contrasts Quickstart

You can attach contrasts directly to
[`hrf()`](https://bbuchsbaum.github.io/fmridesign/reference/hrf.md)
...
To build a full design matrix, bind the event regressors with the
baseline regressors.
...
#> [7] "stim_task_stim.tool_task.ignore"   "stim_task_stim.object_task.ignore"
```

Downstream GLM fitting occurs in analysis packages (e.g., fmrireg), but
fmridesign provides consistent, inspectable design matrices you can feed
into those tools.
